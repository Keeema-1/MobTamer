...
execute unless entity @e[team=!,tag=mt.selected,tag=mt.player_check,tag=mt.pet] unless predicate mobtamer:looking_at/player_check if entity @s[tag=!mt.selected] run scoreboard players set $mt.player_select_check mt.score 1
