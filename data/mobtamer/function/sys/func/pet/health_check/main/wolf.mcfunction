...
execute store result score #mt.damage mt.temp run data get entity @s AbsorptionAmount 100
...
# execute unless score #mt.damage mt.temp matches 0 run tellraw @a [{"score":{"name":"#mt.damage","objective":"mt.health"}}]
...
execute unless score @s mt.health matches 1.. run function mobtamer:sys/func/pet/health_check/down
