...
execute if score $mt.check mt.score matches 0 run data modify storage mobtamer:temp data.player_settingsbaby_grow set value 1b
execute if score $mt.check mt.score matches 0 run tellraw @s ["",{"text": "  "},{"text": "子供ペットの成長","color": "green","hoverEvent": {"action": "show_text","value": [{"text":"子供のペットの成長を有効とするか否か"},{"text":"\n\nデフォルト：無効","color":"gray"}]}},{"text": "","color": "yellow"},{"text": " ＜有効＞ ","color": "aqua"},{"text": "[変更]","color": "light_purple","clickEvent": {"action":"run_command","value": "/trigger mt.trigger set 52"}}]
execute if score $mt.check mt.score matches 1 run data modify storage mobtamer:temp data.player_settingsbaby_grow set value 0b
execute if score $mt.check mt.score matches 1 run tellraw @s ["",{"text": "  "},{"text": "子供ペットの成長","color": "green","hoverEvent": {"action": "show_text","value": [{"text":"子供のペットの成長を有効とするか否か"},{"text":"\n\nデフォルト：無効","color":"gray"}]}},{"text": "","color": "yellow"},{"text": " ＜無効＞ ","color": "gray"},{"text": "[変更]","color": "light_purple","clickEvent": {"action":"run_command","value": "/trigger mt.trigger set 52"}}]
