...
summon illusioner ~ ~ ~ {Tags:["mt.new"]}

execute as @e[type=illusioner,tag=mt.new,limit=1] run function mobtamer:sys/player/summon/illusioner_sub

tag @e[type=illusioner,tag=mt.new,limit=1] remove mt.new
