...
execute as @e[type=ghast,team=!,tag=mt.player_check,tag=mt.pet,tag=!mt.stop,tag=!mt.down] unless predicate mobtamer:targeting unless score @s mt.hostile_interval matches 1.. at @s run function mobtamer:sys/player/advancement/player_hurt_entity/any/guardian
