...
execute unless score $mt.controller mt.slot matches ..-1 as @e[team=mt.common,tag=mt.selected,tag=mt.player_check,tag=mt.pet] run title @a[tag=mt.player_check] actionbar ["",{"text":"【モード：騎乗】 ","color": "dark_red"},{"text":"（上）"},{"text":" "},{"selector":"@e[team=mt.common,tag=mt.check,limit=1]"},{"text":" → （下）"},{"selector":"@s"},{"text":" （右クリックで決定）"}]
...
execute if score $mt.controller mt.slot matches ..-1 as @e[team=mt.common,tag=mt.selected,tag=mt.player_check,tag=mt.pet] run title @a[tag=mt.player_check] actionbar ["",{"text":"【モード：騎乗】 ","color": "dark_red"},{"text":"（上）"},{"text":" "},{"selector":"@a[tag=mt.player_check,limit=1]"},{"text":" → （下）"},{"selector":"@s"},{"text":" （右クリックで決定）"}]
