...
execute if data storage mobtamer:temp data.player.SelectedItem.components."minecraft:entity_data"{Tags:["mt.pet"]} run scoreboard players set @s mt.summon_check_time 3
execute unless score @s mt.summon_check_time matches 3 if data storage mobtamer:temp data.player.Inventory[{Slot:-106b}].components."minecraft:entity_data"{Tags:["mt.pet"]} run scoreboard players set @s mt.summon_check_time 3
