...
execute if entity @s[tag=mt.selected] run title @a[tag=mt.player_check] actionbar ["",{"text":"【モード：情報】 ","color": "green"},{"text":"＜選択中＞ ","color": "aqua"},{"selector":"@s"},{"text":"  ♥","color": "red"},{"score":{"name": "$mt.display","objective": "mt.health"},"color": "green"},{"text":"/"},{"score":{"name": "$mt.display","objective": "mt.max_health"}},{"text":"   所持Exp ","color": "green"},{"score":{"name": "@s","objective": "mt.xp"},"color":"green"}]
