...
type_database_path = 'entity_type.csv'
...
with open(type_database_path, encoding='utf-8-sig', newline='') as f:
    reader = csv.reader(f)
    type_database = [row for row in reader]
    headings = type_database[0]
    type_database = [row for row in type_database if row[type_database[0].index('Mob')]=='1']
...
def get_column(heading):
    ...
    return [row[idx] for row in type_database]

def write_entity_types_json(output_list, path):
    output = {"replace":False, "values":output_list}
    path = '../data/mobtamer/tags/entity_type/' + path + '.json'
...
def get_column_as_np(heading):
    ...
    return np.array([float(row[idx]) if is_num(row[idx]) else np.nan for row in type_database])
...
if(1):
    ...
    write_entity_types_json(tmp_list, 'mob')
...
if(1):
    tmp_list = filter_name_list(get_column_as_np('Fire Resistance') == 1)
    # tags/entity_types/burn_in_the_sun
    write_entity_types_json(tmp_list, 'fire_resistance')

if(1):
    tmp_list = filter_name_list(get_column_as_np('Breathe Underwater') == 1)
    # tags/entity_types/burn_in_the_sun
    write_entity_types_json(tmp_list, 'breathe_underwater')

if(1):
    tmp_list = filter_name_list(get_column_as_np('Can Swim') == 1)
    # tags/entity_types/burn_in_the_sun
    write_entity_types_json(tmp_list, 'can_swim')

if(1):
    other_filter = np.full(len(type_database), True)

    filter = (get_column_as_np('Width') > 1.0)
    hitbox_list = filter_name_list(filter)
    write_entity_types_json(hitbox_list, 'size/wide')
    ...
    hitbox_list = filter_name_list(filter)
    write_entity_types_json(hitbox_list, 'size/tall')
    other_filter = np.logical_not(filter) & other_filter
    print('tall')
    print(hitbox_list)
    print()
