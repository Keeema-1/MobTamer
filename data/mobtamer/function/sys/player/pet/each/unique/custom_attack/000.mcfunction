...
execute store result storage mobtamer:temp data.rot2vec.Pos[0] double 0.000075 run scoreboard players get $mt.dx mt.score
execute store result storage mobtamer:temp data.rot2vec.Pos[1] double 0.000075 run scoreboard players get $mt.dy mt.score
execute store result storage mobtamer:temp data.rot2vec.Pos[2] double 0.000075 run scoreboard players get $mt.dz mt.score
...
execute if entity @s[type=!#mobtamer:breathe_underwater,type=!#mobtamer:can_fly] if data storage mobtamer:temp data.pet{OnGround:1b} rotated ~ 0 positioned ^ ^ ^0.5 unless block ^ ^ ^ #mobtamer:space align xyz positioned ~-0.5 ~ ~-0.5 if entity @s[dx=1,dy=0,dz=1] run data modify entity @s Motion[1] set value 0.4d
