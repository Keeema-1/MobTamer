...
# 足元が危険ブロックのとき
execute if entity @s[type=!#mobtamer:fire_resistance] if block ~ ~-0.5 ~ #mobtamer:dangerous if predicate mobtamer:random/100m run function mobtamer:sys/common/pet/tp/0
execute if entity @s[type=#mobtamer:fire_resistance] if block ~ ~-0.5 ~ #mobtamer:dangerous_fire_resistance if predicate mobtamer:random/100m run function mobtamer:sys/common/pet/tp/0
# 離れたら
