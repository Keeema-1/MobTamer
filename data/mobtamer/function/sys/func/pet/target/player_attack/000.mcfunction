tag @s add mt.target
execute as @e[team=!,type=#mobtamer:can_revenge,tag=mt.player_check,tag=mt.pet,tag=!mt.stop,tag=!mt.down] unless predicate mobtamer:targeting at @s run function mobtamer:sys/func/pet/target/main/0000
