...
execute if entity @e[team=mt.common,tag=mt.selected,tag=mt.player_check,tag=mt.pet] as @e[team=mt.common,tag=mt.selected,tag=mt.player_check,tag=mt.pet,limit=1] run function mobtamer:sys/player/item/conductor/title/info/01
