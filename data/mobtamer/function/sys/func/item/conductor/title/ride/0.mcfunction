...
execute on vehicle if entity @s[tag=mt.player_check,tag=mt.pet] run function mobtamer:sys/func/item/conductor/title/ride/01
execute on vehicle if entity @s[tag=mt.player_check,tag=mt.pet] run return 1

execute if entity @e[team=!,tag=mt.player_check,tag=mt.pet] run function mobtamer:sys/func/item/conductor/title/ride/00
